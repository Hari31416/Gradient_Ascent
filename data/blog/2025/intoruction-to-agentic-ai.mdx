...
However, at their core, they are just a system that takes text (and sometimes images, video and audio) as input and generates text (and other formats) as output. They lack the ability to interact with the environment, fetch information from the web, or access external databases. Morever, they are limited to the information on which they are trained and can not fetch the latest information. Techniques, like retrival-augmented generation can solve some of these problems, but, unless LLMs are not able to take actions based on the information they have, they will always be limited.
...
<figure>
  ...
  <figcaption>
    ...
    can not take actions.
  </figcaption>
</figure>

Agents are a way to overcome these limitations. In this article, we will discuss what agents are, how they work, the benefits of using agents and some applications of agentic AI.

## What Exactly are Agents?

There is not a universal definition of an agent. In fact, it is even debatable whether there is a hard line that seperates a system from not being an agent to being an agent. To quote Andrew NG[^5]:
...
In this spirit, instead of asking whether a system can be called an agent or not, we can ask how agentic a system is. There are different level of "ageticness". A system is more "agentic" if, by use of an LLM, it is able to complete a task more autonomously. For example, a system that lets to book a filght between two cities is more agentic than a system that just gives you the list of available flights. Neverthless, we must have some definition of an agent to work with. I like the definition given by Harrison Chase, cofounder of Langchain[^6]:
...
Based on how much control an LLM has over the workflow of you application, we can categorize the system into different levels of agenticness. Here are some of the levels of agenticness:
...
- **Level 1: Router:** Here, you have a predefined workflow and only agency that an LLM have is to the route your workflow will take. For example, a system that takes a user query, decides in which category the query falls and calling the appropriate function.
...
<table>
<caption>Various levels of agenticness (Table from[^8])</caption>
...
This section outlines a workflow of a typical agentic AI system. An agent consists of two main components:
...
Apart from these, the system has access to a state that can be read/write by the LLM. The state can be used to store information that can be used by the LLM to make decisions like what actions to take next and when to stop the task. To solve a task, the agent uses ReAct framework [^7]. The ReAct framework is based on two steps: Reason and Action. At each step, the system reasons over the current state and takes an action based on the reasoning.
...
The agent starts when a human gives it a prompt. The LLM utilizes its reasoning capabilities and the current state to take an action (like calling a tool at its deisposal). The action results in a change in the state and the LLM generates a response based on the new state. This process continues until the LLM decides that the task is completed or a pre-defined number of steps have been taken. At any step, the LLM can ask for clarification, feedback or more information from the human. Human can also intervene at any step to end the task.

<figure>
  <video controls className="md:w-3/4">
    <source src="/static/images/2025/agentic_ai/hf_calculator_agent.mp4" type="video/mp4" />
  </video>
  <figcaption>
...
1. **Break down the task:** The first step is to break down the task into smaller sub-tasks. Each sub-task should be simple enough that it can be solved by an LLM with conjunction with a tool or two.
...
4. **State Management:** Create a state that can be read/write by the LLM. You must implement a way to read and update the state as required by the LLM.
5. **Run the ReAct Loop:** Run the ReAct loop.

Note that these tasks are not trivial to code. Fortunately, you do not need to start from scratch. There are libraries like `langchain`, `smolagents`, `pydantic-ai`, `crewAI` etc. implements all the necessary tools and classes to create an agent.
...
- **Resource Intensiveness**: Using a single LLM in itself is very resource intensive. Agentic AI systems requires multiple LLMs, tools, and agents, making them even more resource-intensive.
...
In this article, we discussed what agents are, how they work, the benefits of using agents, and some applications of agentic AI. We also discussed the workflow of a typical agent and how to create a simple agent. We briefly touched the topic of challenges involved in creating and running an agentic system. We hope this article has given you a better understanding of agentic AI and how it can be used to solve complex problems. If you want learn how to create your own agent using python, hop on to the [next article](https://blogs.hari31416.in/blog/2025/create-your-own-agent).
...
[^5]: [The Batch Weekly Issues 253 (Deeplearning.AI)](https://www.deeplearning.ai/the-batch/issue-253/)

[^6]: [What is an AI agent? (Langchain)](https://blog.langchain.dev/what-is-an-agent/)
