...
There is not a universal definition of an agent. In fact, it is even debatable whether there is a hard line that seperates a system from not being an agent to being an agent. To quote Andrew NG[^1]:
...
Apart from these, the system has access to a state that can be read/write by the LLM. The state can be used to store information that can be used by the LLM to make decisions like what actions to take next and when to stop the task. To solve a task, the agent uses ReAct framework [^4]. The ReAct framework is based on two steps: Reason and Action. At each step, the system reasons over the current state and takes an action based on the reasoning.
