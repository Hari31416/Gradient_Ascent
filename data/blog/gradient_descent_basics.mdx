...
Gradient Descent (GD) is a fundamental optimization algorithm used in machine learning to minimize a function. It is used to find the minimum of a function by iteratively moving toward the steepest descent. GD is the *secret sauce* behind the success of many machine learning algorithms and neural networks.
...
We need to start from the basics to understand what this equation and the various terms mean.
...
Now that we have a basic understanding of the terms, we are ready to understand the equation (1). Let $f(\theta)$ be a function with parameters $\theta$ that we need to optimize. Note that $\theta$ can be a scalar or a vector. For this, the GD algorithm is:
...
5. Break if a specified condition is met. The most common conditions are:
   ...
   2. Define a small number $\epsilon$ (threshold) and break when $||\theta_{t+1} - \theta_t|| < \epsilon$, that is, break when the parameters stop changing significantly.
...
x = 1.8 - 0.3\times 1.6 = 1.32 \tag{b}
$$

$$
f'(1.32) = 2\times1.32 - 2 = 0.64\\
x = 1.32 - 0.3\times 0.64 = 1.128 = 1.13 \tag{c}
$$
...
<div class="flex flex-wrap">
  ...
  <div class="-mt-10">
    <small class="text-center">
      ...
      for 0.1, 16 for 0.8 but only 5 for 0.4). Using a large learning rate (0.8) results in the
      algorithm or oscillation before reaching the global minimum.
    </small>
  </div>
</div>
